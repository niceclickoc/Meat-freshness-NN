...
for i in range(len(labels)):
    chromatic_classes = label_encoder.inverse_transform(np.arange(len(chromatic_preds[i])))
    hog_classes = label_encoder.inverse_transform(np.arange(len(hog_preds[i])))
    depth_map_classes = label_encoder.inverse_transform(np.arange(len(depth_map_preds[i])))
    print("Хроматическое предсказание:")
    for cls, prob in zip(chromatic_classes, chromatic_preds[i]):
        print(f"Класс: {cls}, Вероятность: {prob}")
        print("Хроматич", chromatic_preds[i])
