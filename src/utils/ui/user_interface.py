...
class UserInterface(QWidget):
    ...
    def init_ui(self):
        ...
        # Кнопка Тестовый режим (сверху)
        self.test_mode_button = QPushButton("Тестовый режим (сделать фото)")
        self.test_mode_button.clicked.connect(self.start_test_mode)
        self.test_mode_button.setFixedWidth(250)
        self.test_mode_button.setStyleSheet("background-color: #f0f0f0; border: 1px solid #ccc; padding: 5px;")
        
        test_mode_layout = QHBoxLayout()
        test_mode_layout.addStretch()
        test_mode_layout.addWidget(self.test_mode_button)
        test_mode_layout.addStretch()
        self.main_layout.addLayout(test_mode_layout)
